...
# By default, detect $(CC)'s architecture - x64 or aarch64. But also allow
# the user to specify a cross-compiled architecture with setting either
# "ARCH" or "arch", in either the environment or make command line.
detect_arch := $(shell echo -e \
                               "x64        x86_64     __x86_64__\n"  \
                               "aarch64    aarch64    __aarch64__\n" \
                       | $(CC) -E -xc - | grep ' 1$$')
host_arch := $(word 1, $(detect_arch))
host_arch_str := $(word 2, $(detect_arch))
ifndef ARCH
    ARCH := $(host_arch)
...
# TODO: if the user defines ARCH, he must also define ARCH_STR (which is
# used below). Was this intended??
...
generated-headers: $(out)/gen/include/bits/alltypes.h $(out)/gen/include/osv/version.h
...
$(out)/loader-stripped.elf: $(out)/loader.elf
	$(call quiet, cp $(out)/loader.elf $(out)/loader-stripped.elf, CP $(out)/loader.elf $(out)/loader-stripped.elf)
	$(call quiet, $(STRIP) $(out)/loader-stripped.elf, STRIP $(out)/loader-stripped.elf)
...
$(out)/gen/include/ctype-data.h: $(out)/gen-ctype-data
	$(call very-quiet, mkdir -p $(out)/gen/include)
...
# We want to run scripts/gen-version-header every time before using
# gen/include/osv/version.h, to see if the version changed. But this script
# does *not* change gen/include/osv/version.h's timestamp unless the version
# actually changed, so we need to use an "order only prerequisite", so as not
# to assume that running the rule means version.h was changed.
$(out)/gen/include/osv/version.h: | perhaps-modify-version-h
